...
export async function POST(req: NextRequest) {
  try {
    ...
    const { text, action = 'improve_wording', roleTitle, jobDescription } = body;
    ...
    const result = await rewriteResumeText(
      text,
      action,
      { field: 'summary', roleTitle, jobDescription },
      aiProvider
    );
    ...
    return NextResponse.json({ error: 'Failed to rewrite summary' }, { status: 500 });
  }
}
